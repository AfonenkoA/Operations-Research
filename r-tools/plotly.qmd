...
1.  Используя функцию [``seq()`](https://rdrr.io/r/base/seq.html) создаём вектор `x` из 100 точек от $0$ до $2\pi$.
2.  Получаем вектор `y` применяя функцию [`sin()`](https://rdrr.io/r/base/Trig.html) к вектору `x` @sec-r-lang-basic-vectorization
3.  Вызываем функцию [`plot_ly()`](https://rdrr.io/cran/plotly/man/plot_ly.html) определяя и передаем данные яным образом @sec-r-lang-basic-function-call-with-args
4.  Параметр `type = 'scatter'` определяет тип графика: по точкам.
5.  Парметр `mode = 'lines'` определяет тип отображения непрерывной линией
:::
...
1.  Используя функцию [`seq()`](https://rdrr.io/r/base/seq.html) создаём вектор `x` из 10 точек от $0$ до $2\pi$
2.  Передаём данные
...
3.  Название перовй линии определяется параметром `name = 'sin(x)'`. Полученная фигура передаётся в функцию [`add_trace()`](https://rdrr.io/cran/plotly/man/add_trace.html) с помощью оператора конвейера @sec-r-tools-basic-pipe.
4.  Параметры [`add_trace()`](https://rdrr.io/cran/plotly/man/add_trace.html) аналогичны параметрам [`plot_ly()`](https://rdrr.io/cran/plotly/man/plot_ly.html).
...
1.  Используя функцию [`seq()`](https://rdrr.io/r/base/seq.html) создаём вектор `x` из 100 точек от $0$ до $\frac{\pi}{2}$.
2.  С помощью функции `outer` @sec-r-tools-basic-outer генерируем матрицу @sec-r-lang-basic-matrix размером `100x100`
3.  Для каждой комбинации `x,y` будет вызвана переданная лямбда функция @sec-r-lang-advanced-lambda . Матрица передаётся с помощью оператора конвейера @sec-r-tools-basic-pipe в функцию транспонирования [`t()`](https://rdrr.io/r/base/t.html).
4.  Матрица транспонируется [`t()`](https://rdrr.io/r/base/t.html) для соответствия осей матрицы осям графика.
...
1.  Используя функцию [`seq()`](https://rdrr.io/r/base/seq.html) создаём вектор `x` из 100 точек от $0$ до $\frac{\pi}{2}$.
2.  С помощью функции `outer` @sec-r-tools-basic-outer генерируем матрицу @sec-r-lang-basic-matrix размером `100x100`
3.  Для каждой комбинации `x,y` будет вызвана переданная лямда функция @sec-r-lang-advanced-lambda . Матрица передаётся с помощью оператора конвейера @sec-r-tools-basic-pipe в функцию транспонирования [`t()`](https://rdrr.io/r/base/t.html).
4.  Матрица транспонируется [`t()`](https://rdrr.io/r/base/t.html) для соответствия осей матрицы осям графика.
5.  Передаём данные для построения, кроме `z` ещё и `x` с `y`.
6.  Используется тип графика контурный.
:::
...
2.  Используя функцию [`seq()`](https://rdrr.io/r/base/seq.html) создаём вектора `x,y,z` из $20$ точек от $-5$ до $5$.
3.  С помощью функции \`tidyr::crossing\` @sec-r-tools-tidyr-crossing генерируем датафрейм @sec-r-tools-tibble размером `20x20x20` в котором будут все комбинации значений переменных `x`, `y` и `z`.
4.  Применяем функцию `f` к каждой строке датафрейма `d` с помощью функции `purrr::pmap_dbl`, @sec-r-tools-purrr-pmap в результате получаем вектор `u`, который содержит значения функции для всех точек её области определения.
5.  Вызываем функцию построения графика [`plot_ly()`](https://rdrr.io/cran/plotly/man/plot_ly.html) и передаём туда датафрейм с координатами точек построения.
