...
fig <- plot_ly(x=x, y=y, z=z, type="contour", ncontours=25, name='F(p)') |> 
  add_trace(
    x = c(-4,-4,4,4),
    y = c(2,3,3,2),
    fill = 'toself',
...
x <- y <- seq(-5, 5, length.out = 100)
z <- outer(x, y, \(p1,p2) { p1^2+p2^2+p1*p2+2*p1+4*p2 +5}) |> slog() |> t()
fig <- plot_ly(x=x, y=y, z=z, type='contour', ncontours=30 , name='F(p)') |> 
  add_trace(
    x = c(-4,-4,4,4),
    y = c(2,3,3,2),
    fill = 'toself',
...
{{< include ../../util/_plotly_show_safe.qmd >}}
